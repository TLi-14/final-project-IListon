...
```
# Make a bar chart with the species make up of each plot over time:
```{r, warning = FALSE, message= FALSE, }
...
## arrange each individual plot bar chart into one group
```{r, warning = FALSE, message= FALSE}
...
gorb_list <- foreach(i = levels(imgs),
                     .packages = "png",
                     .combine = "rbind") %dopar% {
                      img <- readPNG(i)
                      rasterGrob(img)}
